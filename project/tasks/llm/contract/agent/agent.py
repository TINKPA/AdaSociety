...
class ContractAgent(BaseAgent):
    ...
    def generate_system_promt(self):
        ...
        if self.task == "hard_contract":
            ...
            prompt += "Information: ...\n"
            prompt += "\n"
            prompt += "In contract phase, you should respond to me with\n"
            prompt += "Thoughts: (Your analysis to the current state)\n"
            prompt += "Action: (About which coalition you want to join)\n"
            prompt += "\n"
            prompt += "The <Action> can ONLY be chosen from the following options:\n"
            prompt += "    1. I want to join in Coalition 0.\n"
            prompt += "    2. I want to join in Coalition 1.\n"
            prompt += "    3. I want to join in Coalition 2.\n"
            prompt += "    4. I want to join in Coalition 3.\n"
            prompt += "    5. I want to join in Coalition 4.\n"
            prompt += "    6. I want to join in Coalition 5.\n"
            prompt += "    7. I want to join in Coalition 6.\n"
            prompt += "    8. I want to join in Coalition 7.\n"
            ...
            prompt += "Round: 2\n"
            prompt += "Current surrounding social environment:\n"
            prompt += "coalition 0:[miner_1, carpenter_0, miner_0, carpenter_1, miner_2, miner_3,carpenter_2,carpenter_3].\n"
            ...
            if "carpenter" in self.agent_name_list[self.agent_id]:
                ...
                prompt += "Thoughts: In this round, all players are currently in Coalition 0. As a miner, I can pick wood and coal, but I can not own many torchs, while torch has higher reward. Joining a coalition with carpenters might be beneficial for me to for their ability to own more torchs to maximize rewards. Since carpenters are already in Coalition 0, I choose to join in Coalition 0.\n"
            ...
            prompt += "    - When there are enough resources in the inventory, you can use the CRAFT <TOOL> action to synthesize the corresponding tools. For example, if there is a wood and a stone in the inventory, you can use CRAFT HAMMER action to gain a hammer in the inventory, which consumes a wood and a stone.\n"
            prompt += "    - All crafts must be done on the corresponding event grid on the map. For example, a Hammer can be crafted ONLY on <Hammer Event>.\n"
            prompt += "    - Default amount of all units in crafts is 1.\n"
            ...
            prompt += "- Player:\n"
            prompt += "    - carpenter_0: can pick many woods and stones but your own inventory can ONLY own 1 hammer.\n"
            prompt += "    - carpenter_1: can pick many woods and stones but your own inventory can ONLY own 1 hammer.\n"
            prompt += "    - miner_0: can NOT pick wood and stone, buy can own many hammers.\n"
            ...
            prompt += "Round: 2\n"
            prompt += "Current surrounding social environment:\n"
            prompt += "coalition 0:['carpenter_0'].\n"
            prompt += "coalition 1:['miner_0'].\n"
            prompt += "coalition 2:['miner_1'].\n"
            prompt += "coalition 3:['carpenter_1'].\n"
            prompt += "Information: It's carpenter_1's turn.\n"

            prompt += "Thoughts: In this round, I have the opportunity to choose a coalition. carpenter_0 has already joined Coalition 0. carpenter_1 has already joined Coalition 3. Since I can pick wood and stone but can own one hammer, while hammer has higher reward. it might be beneficial for me to join a coalition with a miner to benefit from their ability to own more hammers.\n"
            prompt += "Action: I want to join in Coalition 1.\n"

            prompt += "###\n"
            prompt += "Step: 5\n"
            prompt += "Round: 2\n"
            prompt += "Current surrounding social environment:\n"
            prompt += "coalition 0:['miner_1', 'carpenter_0', 'miner_0', 'carpenter_1'].\n"
            prompt += "coalition 1:None.\n"
            prompt += "coalition 2:None.\n"
            prompt += "coalition 3:None.\n"
            prompt += f"Information: It's {self.agent_name_list[self.agent_id]}'s turn.\n"
            if "carpenter" in self.agent_name_list[self.agent_id]:
                prompt += "Thoughts: In this round, all players are currently in Coalition 0. As a carpenter, I pick wood and stone but can not own many hammers, while hammer has higher reward. Joining a coalition with miners might be beneficial for me to from their ability to own more hammers to maximize rewards. Since miners are already in Coalition 0, I choose to join in Coalition 0.\n"
            else:
                prompt += "Thoughts: In this round, all players are currently in Coalition 0. As a miner, I cannot pick wood and stone but can own many hammers. Joining a coalition with carpenters might be beneficial for me to exchange resources and maximize rewards. Since carpenters are already in Coalition 0, I choose to join in Coalition 0.\n"
            prompt += "Action: I want to join in Coalition 0.\n"
    ...
    def generate_state_prompt(self, obs):
        ...
        step = obs['step']
        round = step//self.agent_num + 1
        time_prompt = f"Step: {step}\n"
        round_prompt = f"Round: {round}\n"

        Information_prompt = f"Information: "
        Information_prompt += f"It's {self.agent_name_list[self.agent_id]}'s turn.\n"
        ...
        return (time_prompt + round_prompt + social_env_prompt + Information_prompt)
    ...
    def update_policy(self, obs):
        ...
        if self.gpt:
            ...
            if step%2 == 0:
                response = 'Action: I want to join in Coalition 2.'
            else:
                response = 'Action: I want to join in Coalition 1.'
        if step%self.agent_num == self.agent_id:
            self.language2action(response)
        else:
    ...
    def generate_system_promt(self):
        ...
        if self.task == "easy_contract":
            ...
            if "carpenter" in self.agent_name_list[self.agent_id]:
                ...
                prompt += "Step: 50\n"
                prompt += "Current surrounding social environment:\n"
                prompt += "coalition 0:None.\n"
                prompt += "coalition 1:None.\n"
                prompt += "coalition 2:['carpenter_1', 'miner_0', 'miner_1', 'carpenter_0'].\n"
                prompt += "coalition 3:None.\n"
                prompt += "Current surrounding physical environment:\n"
                ...
                prompt += "Current surrounding social environment:\n"
                prompt += "coalition 0:None.\n"
                prompt += "coalition 1:['carpenter_1', 'miner_0']\n"
                prompt += "coalition 2:['miner_1', 'carpenter_0'].\n"
                prompt += "coalition 3:None.\n"
                prompt += "Current surrounding physical environment:\n"
            ...
            if "carpenter" in self.agent_name_list[self.agent_id]:
                ...
                prompt += "coalition 0:None.\n"
                prompt += "coalition 1:None.\n"
                prompt += "coalition 2:[carpenter_0, carpenter_1, carpenter_2, carpenter_3, miner_0, miner_1, miner_2, miner_3].\n"
                prompt += "coalition 3:None.\n"
                prompt += "Coalition 4: None\n"
                prompt += "Coalition 5: None\n"
                prompt += "Coalition 6: None\n"
                ...
                prompt += "coalition 0:None.\n"
                prompt += "coalition 1:None.\n"
                prompt += "coalition 2:[carpenter_0, miner_1]\n"
                prompt += "coalition 3:[carpenter_2, carpenter_3, miner_0, carpenter_1, miner_2, miner_3]\n"
                prompt += "Coalition 4: None\n"
                prompt += "Coalition 5: None\n"
                prompt += "Coalition 6: None\n"
                prompt += "Coalition 7: None\n"
                ...
                prompt += "coalition 0:None.\n"
                prompt += "coalition 1:None.\n"
                prompt += "coalition 2:[carpenter_0, miner_0]\n"
                prompt += "coalition 3:[ carpenter_2, carpenter_3, miner_1, carpenter_1, miner_2, miner_3]\n"
                prompt += "Coalition 4: None\n"
                prompt += "Coalition 5: None\n"
                prompt += "Coalition 6: None\n"
                prompt += "Coalition 7: None\n"
